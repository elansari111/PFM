...
const StudentScheduleCalendar = () => {
  const [view, setView] = useState('timeGridWeek');
  const calendarRef = useState(null);

  const { data: schedules, isLoading } = useQuery({
    queryKey: ['student-schedule'],
    queryFn: () => api.get('/student/schedule').then(res => res.data.schedule),
    refetchInterval: 30000, // Refresh every 30 seconds for real-time updates
  });

  const events = schedules?.map(schedule => ({
    id: schedule.id,
    title: `${schedule.module.name} - ${schedule.module.teacher.user.name}`,
    ...
    extendedProps: {
      ...
      teacher: schedule.module.teacher.user.name,
      type: schedule.type,
      module: schedule.module.name,
    },
  })) || [];

  // Helper function to get the next occurrence of a day/time
  function getNextOccurrence(dayOfWeek, time) {
    const days = ['', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
    const now = new Date();
    const targetDay = dayOfWeek; // 1-7 (Monday-Sunday)
  ...
  // Generate consistent colors for modules
